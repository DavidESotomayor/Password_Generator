...
function generatePassword() {
    ...
    for (var i = 0; i < passwordLength; i++) {
        var randomCharacterType = characterTypeArray[Math.floor(Math.random() * (characterTypeArray.length))];
        var randomKey = characterObject[randomCharacterType];
        randomizeArray = randomKey[Math.floor(Math.random() * (randomKey.length))];
